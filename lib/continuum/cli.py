...
from continuum.command import (
    Command,
    CommandError,
    ClashingCommandNames,
)

from continuum.util import (
    iterable,
    ensure_unique,
...
from continuum.invariant import (
    Invariant,
)
...
class CLI(object):
    ...
    def __init__(self, *args, **kwds):
        ...
        for name in self.module_names:
            if name == '-continuum':
                ...
                self.module_names.remove('-continuum')
            elif name == 'continuum':
                include_continuum = False

        if include_continuum:
            self.module_names.insert(0, 'continuum')
    ...
    def version(self, args=None):
        sys.stdout.write(add_linesep_if_missing(continuum.__version__))
        return self._exit(0)
...
def run(*args_):
    ...
    continuum.config._clear_config_if_already_created()
...
if __name__ == '__main__':
    # Intended invocation:
    #   python -m continuum.cli <program_name> <library_name> \
    #                           <command_name> [arg1 arg2 argN]
    # i.e. python -m continuum.cli dstk dstoolkit gdcf -f ...
    ...
    nprocs = cpu_count()
    if parallelism_hint:
        assert parallelism_hint <= nprocs, (parallelism_hint, nprocs)
